---
id: testnet
title: Testnet
sidebar_label: Testnet
description: "Strumenti, indirizzi e link utili per l'implementazione della testnet."
keywords:
  - docs
  - polygon
  - nightfall
  - testnet
  - address
image: https://matic.network/banners/matic-network-16x9.png
---

Polygon Nightfall è ora live su **Goerli**.

## Strumenti {#tools}

:::note

Per interagire con il wallet Nightfall, ti occorre un wallet [Metamask](https://metamask.io/).

:::

- [Web wallet](https://wallet.testnet.polygon-nightfall.technology) per inviare transazioni a Nightfall Rollup

## Proponenti {#proposers}

- **Indirizzo preponente**:

```bash
    0xCaE0ed659d7821b59bBFd1b6B79260051e5E9111
```
- **URL proponente (healthcheck)**:

[https://proposer.testnet.polygon-nightfall.technology/healthcheck](https://proposer.testnet.polygon-nightfall.technology/healthcheck)


- **Indirizzo challenger**:

```bash
    0xa55ec93544dd357c14914f67d04da72dBdE6A688
```

## Indirizzi del contratto {#contract-addresses}

- **Shield**:

```bash
   0x2eEeeC446F92c868220CAB1A2ff3cd77937e6560
```

- **Stato**:

```bash
    0xB7262d293D1cAF4F99DB4468CD5882f06e959A62
```


- **Proponenti**:

```bash
    0x14d5A90408E6a2d99fd57CF1706Ef0643122aA4d
```


- **Challenge**:

```bash
    0x77342c9ee6D9085Ef42FC7400f073FA0f361cF78
```
